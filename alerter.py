...
def _build_body(
    ...
    lines = [
        sep,
        f"MA Monitor Daily Scan — {date_str}",
        sep,
    ]
    ...
    if sma_alerts:
        ...
        for e in sma_alerts:
            ...
            lines.append(
                f"{e.ticker:<6} {e.event_type:<20} SMA {e.ma_period}"
    ...
    if atr_alerts:
        ...
        for e in atr_alerts:
            lines.append(
                f"{e.ticker:<6} {e.action:<7} {e.volatility_tier:<6}"
                f"  ATR: {e.atr_pct:.2f}%"
    ...
    lines.append("")
    lines.append(sep)
    lines.append(f"{total} alert(s) detected across {tickers_scanned} tickers scanned.")
    return "\n".join(lines)
...
def send_alert(
    ...
    """Send a single summary email covering SMA and ATR alerts.
